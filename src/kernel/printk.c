...
static void
move_cursor_position(int x, int y);
static void
move_cursor_position(int x, int y)
{
...
static bool_t
is_cursor_left_out_of_bound(void);
static bool_t
is_cursor_left_out_of_bound()
{
  return cursor_x() < 0 ? true : false;
}

static bool_t
is_cursor_upper_out_of_bound(void);
static bool_t
is_cursor_upper_out_of_bound()
{
  return cursor_y() < 0 ? true : false;
}

static bool_t
is_cursor_right_out_of_bound(void);
static bool_t
is_cursor_right_out_of_bound()
{
  return (cursor_x() >= (screen_width() / char_width())) ? true : false;
}

static bool_t
is_cursor_down_out_of_bound(void);
static bool_t
is_cursor_down_out_of_bound()
{
...
static void
set_char_size(int width, int height);
static void
set_char_size(int width, int height)
{
...
static void
set_screen_buffer(unsigned int* buffer_address, unsigned long length);
static void
set_screen_buffer(unsigned int* buffer_address, unsigned long length)
{
...
init_screen(int width, int height)
{
  ...
  set_screen_buffer(
    (unsigned int*)FRAME_BUFFER_MAPPED_ADDRESS,
    (screen_width() * screen_height() * ONE_PIXEL_32_BITS_WIDTH + PAGE_4K_SIZE -
     1) &
      PAGE_4K_MASK);
}
 ...
 */
static char*
itoa(char* str, long num, int base, int size, int precision, int type);
static char*
itoa(char* str, long num, int base, int size, int precision, int type)
{
  char c, sign, tmp[50];
  ...
  for (str = buf; *fmt; fmt++) {
    ...
    switch (*fmt) {
      ...
      case 'o':
        ...
        if (qualifier == 'l')
          str = itoa(
            str, va_arg(args, unsigned long), 8, field_width, precision, flags);
        else
          str = itoa(
            str, va_arg(args, unsigned int), 8, field_width, precision, flags);
        break;
      ...
      case 'X':
        ...
        if (qualifier == 'l')
          ...
          str = itoa(
            str, va_arg(args, unsigned int), 16, field_width, precision, flags);
        break;
      ...
      case 'u':
        ...
        if (qualifier == 'l')
          ...
          str = itoa(
            str, va_arg(args, unsigned int), 10, field_width, precision, flags);
        break;
  ...
  for (count = 0; count < len || line; count++) {
    ...
    if (*offset((unsigned char*)buffer, count) == '\n') {
      ...
      if (is_cursor_left_out_of_bound()) {
        ...
        if (is_cursor_upper_out_of_bound()) {
          new_y_position =
            (screen_height() / char_height() - 1) * char_height();
          move_cursor_position(cursor_x(), new_y_position);
        }
      }
      putchar(screen_buffer(),
              screen_width(),
              cursor_x() * char_width(),
      ...
      line =
        ((cursor_x() + TAB_HOLD_CHARS) & ~(TAB_HOLD_CHARS - 1)) - cursor_x();
    ...
    Label_tab:
      ...
      putchar(screen_buffer(),
              ...
              front_color,
              back_color,
              *offset((unsigned char*)buffer, count));
      cursor_next_column();
    }
