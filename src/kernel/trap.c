...
/*
The value of stack pointer register RSP (the value of RSP is stored in RDI register as a parameter by exception handling module) 
is indexed up to 0x98 bytes to obtain the rip register value at the execution context of the interrupted program and take it as the address of generating exception instruction.
*/

/*
error code format: 32-bit

|----(31 to 16 bit)------|-------(15 to 3 bit)------|--(2 bit)--|--(1 bit)--|--(0 bit)--|

31-61 bit: reserved
15-3 bit: segment selector
2 bit: TI, This bit is set only when the IDT flag bit is unset. If this bit is set, the segment selector of the error code records the segment descriptor or gate descriptor in the local descriptor table (LDT). 
        This flag unset means that it records the global descriptor table (GDT) descriptor
1 bit: IDT, If this bit is set, the segment selector of the error code records the gate descriptor in the interrupt descriptor table(IDT), 
        and unset that it records the descriptor in the descriptor table (GDT / LDT)
0 bit: EXT, If this bit is set, the exception is triggered during the delivery of an external event to the program
*/
...
|----------(31 to 5 bit)----------|--(4 bit)--|--(3 bit)--|--(2 bit)--|--(1 bit)--|--(0 bit)--|

31 to 5 bit: Reserved
4 bit:  I/D, if the flag is unset means that no exception was thrown while getting the instruction, 
            if flag is set, means that have exception was thrown while getting the instruction
3 bit: RSVD, if the flag is unset means that the reserved bit of the page table entry did not throw an exception, 
            if flag set, means that the reserved bit of the page table entry being set  cause to  throw an exception
2 bit: U/S, if the flag is unset means that using super user rights to access the page throw an exception
            if flag set means that using normal user rights to access the page throw an exception
1 bit: W/R, if the flag is unset means that read page, and the flag is set means that write page
0 bit: P, if the flag is unset means that the page is not present, and the flag is set means that page protection
*/
