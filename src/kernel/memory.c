...
init_memory()
{
  ...
  for (unsigned long i = 0; i < global_memory_descriptor.e820.number_entries;
    ...
    if (global_memory_descriptor.e820.entries[i].type == E820_RAM) {
      start =
        PAGE_2M_UPPER_ALIGN(global_memory_descriptor.e820.entries[i].address);
      end =
        PAGE_2M_LOWER_ALIGN(global_memory_descriptor.e820.entries[i].address +
                            global_memory_descriptor.e820.entries[i].length);
      if (end > start) {
  ...
  // needed)
  global_memory_descriptor.bits_map =
    (unsigned long*)PAGE_4K_UPPER_ALIGN(global_memory_descriptor.end_brk);
  ...
  global_memory_descriptor.bits_size =
    BYTES_NUM_TO_PAGE_2M_NUM(end_addr_of_physical_space);

  static const unsigned long LONG_TYPE_BYTES = sizeof(long);
  global_memory_descriptor.bits_length =
    BITS_NUM_UPPER_ALIGN(global_memory_descriptor.bits_size, LONG_TYPE_BYTES);
  ...
  unsigned long pages_addr =
    (unsigned long)(global_memory_descriptor.bits_map +
                    global_memory_descriptor.bits_length);
  global_memory_descriptor.pages =
    (struct Page*)PAGE_4K_UPPER_ALIGN(pages_addr);
  global_memory_descriptor.pages_size =
    BYTES_NUM_TO_PAGE_2M_NUM(end_addr_of_physical_space);
  ...
  unsigned long zones_addr =
    (unsigned long)(global_memory_descriptor.pages +
                    global_memory_descriptor.pages_length);
  global_memory_descriptor.zones =
    (struct Zone*)PAGE_4K_UPPER_ALIGN(zones_addr);
  ...
  global_memory_descriptor.zones_length =
    BYTES_NUM_UPPER_ALIGN(INIT_ZONES_NUMBER * PER_ZONE_BYTES, LONG_TYPE_BYTES);
    ...
    if (global_memory_descriptor.e820.entries[i].type == E820_RAM) {
      ...
      unsigned long end =
        PAGE_2M_LOWER_ALIGN(global_memory_descriptor.e820.entries[i].address +
                            global_memory_descriptor.e820.entries[i].length);
      if (end > start) {
        struct Zone* new_zone =
          global_memory_descriptor.zones + global_memory_descriptor.zones_size;
        ...
        new_zone->pages =
          global_memory_descriptor.pages + BYTES_NUM_TO_PAGE_2M_NUM(start);
        ...
        for (unsigned long j = 0; j < new_zone->number_pages;
          ...
          // unused
          unsigned long target_bitmap_value =
            *(global_memory_descriptor.bits_map +
              ADDR_TO_PAGE_2M_64BITS_LOWER_INDEX(page_index->physical_address));
          target_bitmap_value ^=
            1UL << ADDR_TO_PAGE_2M_NORMAL_INDEX(page_index->physical_address) %
                     64;
        }
      }
    }
